/* ============================================================
   ...
   Character/space offsets come from the Tweaks panel via window.__mo_score.
   ============================================================ */
(function () {
  "use strict";
  ...
  // character offsets (Tweaks): VOID = colder + sparser · DEEP = warmer + denser
  var CHARACTER = {
  ...
  window.addEventListener("mo:section", function (e) {
    ...
    if (s && s !== cur) {
      ...
      if (on()) CF.carrierAccent(s === "contact" ? 0.9 : 0.3);
    }
  ...
  /* ── ANAMORPH LOCK — finding the hidden 0x00 rings the whole lattice ── */
  window.addEventListener("mo:anamLock", function () {
    if (!on()) return;
    ["0x01", "0x02", "0x04", "0x07", "0x02"].forEach(function (a, i) {
      setTimeout(function () { if (on()) CF.pingNode(a, 0.12); }, i * 110);
    });
    setTimeout(function () { if (on()) CF.carrierAccent(1); }, 640);
  });
})();
