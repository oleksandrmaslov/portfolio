/* ============================================================
   M.O. — CINEMATIC LAYER driver (Landing v9)
   ...
   2) GLOBAL WEIGHT — a rAF loop that lerps a smoothed scroll value,
      ...
      the v9 CSS motion-blur consumer was removed in v11). Pure
...
(function () {
  ...
  /* ---------- 1b. v10 SECTION REVEALS — replayable rise+blur ----------
     The about-dive centre stack gets the same staggered entrance as the
     title, re-armed every time the section leaves the viewport so the
     reveal replays on every approach (award sites never enter twice the
     same dead way). Classes are removed on exit — natural state = visible,
     so a React re-render can never strand content hidden. */
  function armSectionReveals() {
    if (reduceMotion) return;
    var SECTIONS = [
      {
        id: "about", stagger: 110,
        sels: [
          ".lp-aboutDive__chrome",
          ".lp-aboutDive__kicker",
          ".lp-aboutDive__title",
          ".lp-aboutDive__lede",
          ".lp-aboutDive__action",
          ".lp-aboutDive__foot",
        ],
      },
      // (v11) NOTE: the live footer is board-flight's .bf-foot, which has its
      // own scroll-driven entrance (footE) — no cinematic reveal needed there.
    ];
    var missing = false;
    SECTIONS.forEach(function (cfg) {
      var root = document.getElementById(cfg.id);
      if (!root) { missing = true; return; }
      if (root.__cinArmed) return;
      root.__cinArmed = true;
      var io = new IntersectionObserver(function (entries) {
        entries.forEach(function (en) {
          var els = [];
          cfg.sels.forEach(function (s) {
            root.querySelectorAll(s).forEach(function (n) { els.push(n); });
          });
          if (en.isIntersecting) {
            els.forEach(function (n, i) {
              n.style.setProperty("--cin-d", (i * cfg.stagger) + "ms");
              n.classList.add("cin-reveal");
            });
          } else {
            els.forEach(function (n) { n.classList.remove("cin-reveal"); });
          }
        });
      }, { rootMargin: "-20% 0px -20% 0px", threshold: 0 });
      io.observe(root);
    });
    if (missing && revealTries++ < 80) setTimeout(armSectionReveals, 120);
  }
  var revealTries = 0;
  ...
  // v11 perf: --cin-k is a static preset — read it ONCE instead of a
  ...
  function frame() {
    ...
    // v11 perf: skip the style writes when the value hasn't visibly moved
  ...
  function boot() {
    ...
    if (window.console) console.log("[cinematic] layer active (intensity 67 · rise+blur)");
  }
