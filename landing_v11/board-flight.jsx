...
function BoardFlight({ onEnter, onContact }) {
  ...
  const [leadUi, setLeadUi] = useBF({ c: 0, o: 0 });
  ...
  useBFE(() => {
    ...
    const update = () => {
      ...
      if (MODE === "takeover") {
        ...
        setLeadUi((prev) => {
          const c = +cVis.toFixed(2), o = +openP.toFixed(2);
          return (prev.c === c && prev.o === o) ? prev : { c, o };
        });
      } else {
    ...
    >
      ...
      <div className="aboutNode-layer" style={{ opacity: leadUi.c.toFixed(3) }} aria-hidden={leadUi.c < 0.02}>
        <div
          ...
          style={{ transform: `scale(${(1 + (leadUi.o || 0) * 0.06).toFixed(3)})` }}
        >
