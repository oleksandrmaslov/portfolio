...
function Universe({ projects = PROJECTS, onActive }) {
  ...
  React.useEffect(() => {
    ...
    projects.forEach((p, i) => {
      ...
      mesh.userData = { project: p, texture: tex, kind: "tile",
    ...
    mount.addEventListener("wheel", (e) => {
      e.preventDefault();
      // forward velocity along look axis (camera local -Z is forward)
    ...
    function handleClick(cx, cy) {
      ...
      if (!hit) return;
      const m = hit.object;
      // Aim the camera at the tile (yaw/pitch toward its world position relative to cam)
      ...
      // Choose nearest equivalent angle to current
      camTarget.yaw   = nearestAngle(camTarget.yaw,   yaw);
      camTarget.pitch = Math.max(-1.45, Math.min(1.45, pitch));
      // Don't slam forward — just aim. Wheel still controls dolly.
      cam.vel = Math.max(cam.vel, 0); // ease any backward into 0
    ...
    function frame(now) {
      ...
      /* idle drift — gentle forward push + slow yaw */
      if (driftActive) {
      ...
      /* wrap every drifting object around the camera */
      for (const m of tiles)   wrapAroundCamera(m, BOX);
      ...
      // For Points, we wrap by adjusting the parent groups — but since these are absolute world
      // positions we shifted, we wrap each star individually only when necessary. Cheap because
      // we only adjust positions, not geometry attributes.

      // wrap stars by repositioning the buffer-attribute relative to camera (rare path)
      wrapPointsAroundCamera(stars,    SBOX);
      wrapPointsAroundCamera(sigStars, SgBOX);

      /* tiles: gently face the viewer with per-card personality offsets */
      for (const m of tiles) {
        ...
        // Apply per-card constant offset so each tile floats at its own angle
        const offQ = new THREE.Quaternion()
          .setFromEuler(new THREE.Euler(
            m.userData.offsetPitch,
            m.userData.offsetYaw,
            m.userData.offsetRoll + Math.sin(now * 0.0006 + m.userData.wobble.p * 6) * 0.04,
            "YXZ"
          ));
        baseQ.multiply(offQ);

        // SOFT slerp — cards re-orient slowly so they feel like floating objects,
        // not aggressive billboards. Camera moves → cards lag, look 3D.
        m.quaternion.slerp(baseQ, 0.06);

        // Band-pass visibility: invisible too close, full in mid, fade out far.
        const dist = m.position.distanceTo(camera.position);
        const nearIn  = THREE.MathUtils.smoothstep(dist, 2.5, 6.0);   // 0→1 fading IN
        const farOut  = THREE.MathUtils.smoothstep(dist, 20, 32);     // 0→1 fading OUT
        m.material.opacity = nearIn * (1 - farOut);
      }
      ...
      for (const sp of ambient) {
        ...
        sp.material.opacity = pulse * nearIn * (1 - farOut);
      }
