...
/* v15 — SINGLE SOURCE: landing_final3/projects-data.js (window.MO_PROJECTS)
...
if (!PROJECTS.length) console.warn("[universe] MO_PROJECTS missing — load landing_final3/projects-data.js first");
...
function Universe({ projects = PROJECTS, onActive, mode = "drift", focusAddr = null }) {
  ...
  React.useEffect(() => {
    ...
    function frame(now) {
      ...
      if (PDRIFT.amp > 0) {
        const wantGain = (mode === "drift" && !dragging && !activeAddr && !hoverObjRef.current)
          ? 1 : PDRIFT.focusDamp;
        pdriftGain += (wantGain - pdriftGain) * (1 - Math.pow(1 - 0.03, dt / 16));
        const s = dt / 1000;
