/* ============================================================
   M.O. SYSTEM — Landing Final 2 · TITLE SCREEN override (v2)
   Loaded AFTER landing_final/landing.jsx — replaces window.TitleScreen.
   Changes vs v1:
...
function useInViewT2(onEnter) {
  const ref = useT2R(null);
  useT2E(() => {
    const el = ref.current;
    if (!el || !onEnter) return;
    const io = new IntersectionObserver((entries) => {
      for (const e of entries) if (e.isIntersecting) onEnter();
...
function TitleScreenV2({ onEnter }) {
  const ref = useInViewT2(onEnter);
  ...
  return (
    <section ref={ref} className={"lp-title lp-title--v1 lp-title--v2 " + (touched ? "is-touched" : "")} id="title" data-screen-label="01 Title">
