...
const PROJECTS = [
  { addr: "0x01", kind: "wafer",  prim: "slab",   file: "Wafer.html",               name: "Wafer",                  sub: "36-key ultrathin split keyboard", year: "2025",   stack: "ZMK · KICAD · NRF52840", color: "#0d1018", model: "models/wafer_demo.glb", modelFit: 5.0, modelPose: { x: -1.15, y: 0, z: 0 }, modelOffset: { x: 0, y: 0.2, z: 0 } },
  ...
  { addr: "0x04", kind: "torch",  prim: "cone",   file: "Tactical-Flashlight.html", name: "Tactical Flashlight",    sub: "For Energy for Ukraine",          year: "12/25",  stack: "C · ARM-M0 · KICAD",     color: "#0e1018", model: "models/tactical_flashlight.glb", assignMaterial: { color: 0x4b5a32, metalness: 0.28, roughness: 0.62 } },
...
function Universe({ projects = PROJECTS, onActive, mode = "drift", focusAddr = null }) {
  ...
  React.useEffect(() => {
    ...
    let flowSm = 0;
    // v14 — ORIGIN→WORK detonation: the formed glyph blasts apart when the
    // transit engages; the field's homing lerp re-gathers the debris.
    let detE = 0, detArmed = false, detDir = null;
    // v14 — WORK→ABOUT portal node: a wireframe node the camera flies
    // THROUGH on the toAbout leg, timed with the iris close.
    const portalGeo = new THREE.WireframeGeometry(new THREE.IcosahedronGeometry(1, 1));
    const portalMat = new THREE.LineBasicMaterial({
      color: 0x35e0c2, transparent: true, opacity: 0,
      blending: THREE.AdditiveBlending, depthWrite: false,
    });
    const portal = new THREE.LineSegments(portalGeo, portalMat);
    ...
    function frame(now) {
      ...
      let _flowDX = 0, _flowDY = 0, _flowDZ = 0;
      {
        ...
        if (_flTransit && !FLOW_RM) {
          ...
          rollTarget = legRoll * 0.055 * bell * styleMul * Math.min(1.3, warpMul);
        }
         ...
         cleanly when you scroll away — no "stuck formed" or "never appeared". */
      {
        ...
        // v14 — arm/fire the detonation once per approach: capture each
        // particle's outward direction from the glyph centre at the moment
        // the origin→work transit begins.
        const segFL = _fl.seg || "";
        if (segFL === "toWork" && !detArmed && formActual > 0.12 &&
            (window.__mo_flightCfg || {}).det !== false && !FLOW_RM) {
          detArmed = true; detE = 1;
          if (!detDir) detDir = new Float32Array(ASM_N * 3);
          const cx = _glyphC.x, cy = _glyphC.y, cz = _glyphC.z;
          const arr0 = assemblyPts.geometry.attributes.position.array;
          for (let i = 0; i < ASM_N; i++) {
            const j = i * 3;
            const dx = arr0[j] - cx, dy = arr0[j+1] - cy, dz = arr0[j+2] - cz;
            const L = Math.sqrt(dx*dx + dy*dy + dz*dz) || 1;
            const m2 = (0.5 + ((i * 2654435761) % 1024) / 1024) / L;
            detDir[j] = dx * m2; detDir[j+1] = dy * m2 + 0.12; detDir[j+2] = dz * m2;
          }
        }
        if (segFL === "origin" || segFL === "toOrigin" || segFL === "title") detArmed = false;
        ...
        // v14 — detonation debris: a decaying outward impulse rides on top of
        // the homing lerp — blast, then re-gather as the reel assembles.
        if (detE > 0.008 && detDir) {
          const push = detE * detE * dt * 0.02;
          const arrD = assemblyPts.geometry.attributes.position.array;
          for (let i = 0; i < ASM_N * 3; i++) arrD[i] += detDir[i] * push;
          detE *= Math.pow(0.90, dt / 16);
        }
        assemblyPts.geometry.attributes.position.needsUpdate = true;

        // Always visible as field; brighter + larger as it forms / ignites.
        const fieldOp = (mode === "ambient" || mode === "grid" || mode === "reel") ? 0.34 : 0.55;
        assemblyPts.material.opacity = Math.min(1, Math.max(fieldOp, 0.2 + formP * 0.8) + igE * 0.4 + arrCollapse * 0.45 + _lvlS * 0.16 + detE * 0.5);
        // v10: the formed glyph keeps near-field particle size — the old
        // +0.055 growth made "0x00" read as soft blobs up close.
        assemblyPts.material.size = 0.1 + formP * 0.015 + igE * 0.5 + arrCollapse * 0.07 + _lvlS * 0.04 + detE * 0.25;
      ...
      /* ── v14 — portal node flythrough on the WORK→ABOUT leg: a wireframe
             node rushes at the camera, swallowing the view just as the iris
             (flight.js) closes through it. Camera-local — can't misframe. ── */
      {
        const want = FLbr.seg === "toAbout" &&
          (window.__mo_flightCfg || {}).portal !== false && !FLOW_RM ? 1 : 0;
        portalOn += (want - portalOn) * (1 - Math.pow(0.88, dt / 16));
        if (portalOn > 0.02) {
          portal.visible = true;
          const tp = Math.max(0, Math.min(1, FLbr.t || 0));
          const te = tp * tp * (3 - 2 * tp);
          _pFwd.set(0, 0, -1).applyQuaternion(camera.quaternion);
          const d = 30 - te * 33;                    // 30u ahead → 3u behind
          portal.position.copy(cam.pos).addScaledVector(_pFwd, d);
          portal.position.y += (1 - te) * 1.2;
          const s = 2.2 + te * te * 26;              // swells as it swallows the view
          portal.scale.set(s, s, s);
          portal.rotation.y = now * 0.00035;
          portal.rotation.x = Math.sin(now * 0.00021) * 0.3;
          const passFade = d < 4 ? Math.max(0.12, d / 4) : 1;
          portalMat.opacity = portalOn * 0.5 * Math.min(1, 0.25 + te * 1.2) * passFade;
        } else {
          portal.visible = false;
        }
      }
