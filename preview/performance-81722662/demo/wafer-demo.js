/* ============================================================
   ...
   production board, the choreographed intro, free-orbit input,
...
(function () {
  ...
  const BOOT_LINES = [
    "WAFER · LIVE DEMO · R3",
    "GEOMETRY · REAL CAD · 51 PARTS",
    "MATRIX · 36 KEYS · ZMK · NRF52840",
  ...
  function onKeyDown(e) {
    if (!active) return;
    if (e.key === "Escape") return;                  // the page layer handles exit
  ...
  function frame(now) {
    ...
    if (st.phase === "intro") {
      if (board.ready) runIntro(dt);   // hold on the far establishing pose until the real GLB parts finish loading
    } else {
  ...
  const api = {
    ...
    start(opts = {}) {
      ...
      emit("phase", "intro");
      if (!board.ready) emit("boot", "GEOMETRY · LOADING REAL PARTS —");
      if (reduced) api.skip();
      return true;
    },
    ...
    renderTick() { try { board.update(16, performance.now()); applyCamera(); renderer.render(scene, camera); } catch (_) {} },
