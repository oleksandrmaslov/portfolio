/* ============================================================
   ...
   pointer wake, screen-paint flow and ripple displacement everywhere.

   - createComposerEffect() plugs into the landing scene's existing composer.
...
(function cursorDistortionModule(global) {
  ...
  function createTextMirror(options) {
    ...
    function drawRun(ctx, text, rect, targetRect, style) {
      ...
      if ("fontStretch" in ctx && style.fontStretch) {
        // Some engines expose computed stretch as percentages (usually "100%"),
        // while the canvas API accepts keywords only. Invalid assignments
    ...
    function makeEntry(element) {
      ...
      if (alphaTag) {
        Object.assign(materialOptions, {
          // Some engines can still blend fully transparent CanvasTexture fragments at
  ...
  function createCursorCore(options) {
    ...
    try {
      ...
      console.warn("[pointer-effects] screen paint unavailable", error);
    }
    ...
    function onWakeMove(event) {
      ...
      // The visible pointer is hidden on touch devices. In mobile Explore, do not
  ...
  function mountStandalone(options) {
    ...
    try {
      ...
      console.warn("[pointer-effects] standalone surface unavailable", error);
      if (mirror) mirror.destroy();
