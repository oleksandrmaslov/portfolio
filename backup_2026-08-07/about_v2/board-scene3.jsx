...
(function () {
  ...
  const CLUSTERS = {
    ...
    crystal: [
      { k:"c",  dx:-3.7, dz: 0,   rot:1, ref:"C3" },
      { k:"c",  dx: 3.7, dz: 0,   rot:1, ref:"C4" },
      { k:"r",  dx: 0,   dz: 3.1, ref:"R5" },
      { k:"tp", dx:-3.7, dz: 3.3, ref:"TP2" },
    ],
    mcu: [
    ...
    cat: [
      { k:"led",   dx:-2.3, dz: 3.1, ref:"LD1" },
      { k:"led",   dx: 2.3, dz: 3.1, ref:"LD2" },
      { k:"r",     dx:-2.3, dz: 4.9, ref:"R6" },
      { k:"r",     dx: 2.3, dz: 4.9, ref:"R7" },
      { k:"sot23", dx: 0,   dz:-4.4, ref:"Q1" },
    ],
    switch: [
  ...
  const LAYOUT = (() => {
    ...
    while (foot.length < 110 && guard++ < 5000) {
    ...
    const vias = []; for (let i = 0; i < 70; i++) { const x = -BOARD_W/2+4 + R()*(BOARD_W-8), z = -BOARD_D/2+3 + R()*(BOARD_D-6); if (inAnt(x, z)) continue; vias.push({ x, z, tent: R() < 0.5 }); }
    const stitch = [];
  ...
  function mainPads(st) {
    ...
    } else if (st.kind === "crystal") { padsFor("xtal", sx, sz, 0).forEach(p => pads.push(p));
  ...
  const COURT = { mcu: [14.6, 15.6], crystal: [9.6, 5.2], usbc: [12.4, 8.4], cat: [7.4, 8.2], switch: [9.4, 7.4] };
  ...
  function drawColor(W, H) {
    ...
    // KERFUR silkscreen cat — line-art mascot at the U2 stop
    {
      const [cx, cz] = TRACE_PTS[STOPS.find(s => s.kind === "cat").p];
      const [u, v] = uv(cx, cz - 0.6, W, H);
      const r = 2.1*S;
      x.strokeStyle = "rgba(230,234,244,0.85)"; x.lineWidth = 0.11*S; x.lineCap = "round";
      x.beginPath(); x.arc(u, v, r, 0, Math.PI*2); x.stroke();
      // ears
      [[-1, -0.55], [1, -0.55]].forEach(([sg]) => {
        x.beginPath();
        x.moveTo(u + sg*r*0.85, v - r*0.5);
        x.lineTo(u + sg*r*1.05, v - r*1.45);
        x.lineTo(u + sg*r*0.25, v - r*0.94);
        x.stroke();
      });
      // eyes (happy arcs)
      [[-0.42], [0.42]].forEach(([sg]) => { x.beginPath(); x.arc(u + sg*r, v - r*0.12, 0.36*S, Math.PI*1.1, Math.PI*1.9); x.stroke(); });
      // nose + tongue out
      x.beginPath(); x.moveTo(u - 0.16*S, v + r*0.22); x.lineTo(u + 0.16*S, v + r*0.22); x.lineTo(u, v + r*0.36); x.closePath(); x.stroke();
      x.fillStyle = "rgba(230,234,244,0.85)";
      x.fillRect(u - 0.17*S, v + r*0.4, 0.34*S, 0.5*S);
      // whiskers
      [[-1], [1]].forEach(([sg]) => { for (let wI = 0; wI < 3; wI++) {
        x.beginPath(); x.moveTo(u + sg*r*0.55, v + r*(0.18 + wI*0.14)); x.lineTo(u + sg*r*1.55, v + r*(0.04 + wI*0.22)); x.stroke();
      }});
      x.fillStyle = "#9aa3b5"; x.font = `600 ${0.5*S}px 'Geist Mono', monospace`; x.textAlign = "center";
      x.fillText("MEOW-DEBUG ONLY", u, v + r + 0.85*S);
    }
    ...
    x.textAlign = "center";
    x.fillText("LOT 2426 · RoHS · ⏚", W/2, H - 1.15*S);
    // barcode (fab lot)
    {
  ...
  function buildComponent(kind) {
    ...
    if (kind === "usbc") {
      ...
      const can = roundedBox(4.6, 1.0, 2.9, 0.2, metal); can.position.y = 0.5 + TOP; grp.add(can);
      const top = roundedBox(4.0, 0.12, 2.3, 0.1, mat(0xd2d8e2, 1.0, 0.18)); top.position.y = 1.06 + TOP; grp.add(top);
      const dot = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, 0.08, 12), mat(0x2a3140, 0.2, 0.5));
      dot.position.set(-1.6, 1.14 + TOP, -0.9); grp.add(dot);
    } else if (kind === "mcu") {
      ...
      // silkscreen-only mascot — the cluster's LEDs are the component
    } else if (kind === "switch") {
