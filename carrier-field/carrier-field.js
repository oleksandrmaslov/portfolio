/* ============================================================
   0x00 CARRIER FIELD — heterodyne sound engine  (build 2026.06)
   ------------------------------------------------------------
   ...
   carrier (alive shimmer) and RESOLVES to zero beat when locked
   ("tuned in"). Every interaction ducks the carrier — you are a
   disturbance in the field, not a sound effect on top of it.

   Standalone. Depends on nothing. Memory-safe:
   - persistent nodes for carrier / probe (no per-frame allocation)
   - transient voices clean themselves up via onended → disconnect
   - hard polyphony cap, one hover-voice per node, single noise buffer
   ============================================================ */
(function () {
  "use strict";
  ...
  // a pleasant pentatonic subset of the lattice for the melodic ambient layer
  const MELODY = ["0x01", "0x02", "0x04", "0x05", "0x07"]; // 9:8 5:4 3:2 5:3 2:1  (G pentatonic)

  // slow-moving harmony for the pad: I–V–vi–IV in G (all consonant w/ carrier).
  // each entry is four partial multipliers of F0 — a chord voicing.
  const CHORDS = [
    [2, 2.5, 3, 4],                 // I   G major   (G B D G)
    [1.5, 1.875, 2.25, 3],          // V   D major   (D F# A D)
    [5 / 3, 2, 2.5, 10 / 3],        // vi  E minor   (E G B E)
    [4 / 3, 5 / 3, 2, 8 / 3],       // IV  C major   (C E G C)
  ];
  ...
  function CarrierField() {
    ...
    function ksStrike(freq, amp, ring, pan, wetAmt) {
      ...
      fb.gain.value = Math.min(0.995, Math.pow(0.001, 1 / (freq * ring))); // −60dB at `ring` seconds
    ...
    function scheduleChord() {
      ...
      chordTimer = setTimeout(function () {
        if (ctx && ctx.state === "running") {
          ...
          setChord(CHORDS[chordIdx], now(ctx), 2.6);    // slow glide between chords
        }
        scheduleChord();
      }, (9 + Math.random() * 7) * 1000);
    }
    ...
    function scheduleMelody() {
      if (!started) return;
      const stepMs = (760 + (1 - idleLife) * 1200) * (0.8 + Math.random() * 0.4);
      melodyTimer = setTimeout(() => {
        melodyStep++;
        const play = ctx && ctx.state === "running" && idleLife > 0.02 && transient < MAX_VOICES - 5 &&
                     hoverVoices.size === 0 && Math.random() < 0.5 + idleLife * 0.4;
        if (play) {
          // walk: mostly stepwise, sometimes a leap
          const leap = Math.random() < 0.18;
          const move = leap ? (Math.random() < 0.5 ? -2 : 2) : ((Math.random() * 3 | 0) - 1);
          melIdx = clamp(melIdx + move, 0, MELODY.length - 1);
          const addr = MELODY[melIdx];
          const hi = (cur.sx > 0.5 ? 1 : 0) ^ (Math.random() < 0.35 ? 1 : 0);
          const oct = hi ? 2 : 1;
          const f = F0 * RATIOS[addr] * oct;
          const pan = (cur.sx - 0.5) * 1.2 + (Math.random() * 0.4 - 0.2);
          bell(f, (0.05 + idleLife * 0.05) * loudnessGain(f), 2.8 + Math.random() * 2.6, pan, 0.8);
          // grace note — a quick neighbour just after (rhythmic variety)
          if (Math.random() < 0.3) {
            const gi = clamp(melIdx + (Math.random() < 0.5 ? 1 : -1), 0, MELODY.length - 1);
            const gf = F0 * RATIOS[MELODY[gi]] * oct;
            setTimeout(() => bell(gf, 0.04 * loudnessGain(gf), 2.4, pan, 0.8), 150 + Math.random() * 120);
          }
          // occasional soft harmony a fifth/octave below
          if (Math.random() < 0.22) {
            const f2 = F0 * RATIOS[addr];
            bell(f2, 0.034 * loudnessGain(f2), 3.4 + Math.random(), -pan, 0.9);
          }
        }
        scheduleMelody();
      }, stepMs);
    ...
    function clickVoice(ratio, pan) {
      ...
      if (samples.lock) {
        ...
        body.frequency.setValueAtTime(F0 * ratio * 2, t);
    ...
    return {
      ...
      wake() {
        ...
        master.gain.exponentialRampToValueAtTime(masterTarget, t + 0.6); // quick, clean fade-in
        state.woken = true;
